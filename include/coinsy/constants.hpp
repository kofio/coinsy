/**
 -----BEGIN PGP MESSAGE-----

=a9Uy
-----END PGP MESSAGE-----
 */

#ifndef COINSEY_CONSTANTS_HPP
#define COINSEY_CONSTANTS_HPP

#include <iostream>
#include <sstream>
#include <string>

namespace coinsy {
  
    namespace constants
    {
        /**
         * The stack version.
         */
        enum { stack_version = 1 };
        
        /**
         * The authentication ip addresses.
         */
        static const char * auth_addresses[2] =
        {
            "72.47.228.69",
            "72.47.228.69"
        };
        
        /**
         * The authentication hostnames.
         */
        static const char * auth_hostnames[2] =
        {
            "coinsy.net",
            "coinsy.org"
        };
    }
}

#endif // COINSEY_CONSTANTS_HPP

/**
 -----BEGIN PGP MESSAGE-----

=a9Uy
-----END PGP MESSAGE-----
 */

#ifndef COINSY_AES_HPP
#define COINSY_AES_HPP

#include <cstdint>
#include <string>

#include <openssl/evp.h>

namespace coinsy {

    /**
     * Implements 256-bit AES.
     */
    class aes
    {
        public:
        
            /**
             * Set the key.
             * @param buf The key buffer.
             * @param len The key length in bytes.
             */
            void set_key(const char *, const std::size_t &);
            
            /**
             * Perform encryption/decryption.
             * @param pt The plaintext.
             * @param ct The ciphertext.
             * @param len The length.
             */
            bool crypt(char *, char *, const std::size_t &);
        
        private:
        
            // ...
        
        protected:
        
            /**
             * The context.
             */
            EVP_CIPHER_CTX ctx_;
            
            /**
             * The counter.
             */
            std::uint8_t counter_[16];
    };
    
} // namespace coinsy

#endif // COINSY_AES_HPP
